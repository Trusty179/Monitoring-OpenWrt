...
return view.extend({
    ...
    render: function(includes) {
        ...
        return startPolling(includes, containers).then(function() {
            ...
            // ===== MONITOR DI BAWAH SYSTEM =====
            var monitor = E('iframe', {
                src: '/luci-static/custom/monitor.html',
                style: 'width:100%;height:500px;border:none;border-radius:12px;margin-top:15px;overflow:hidden;'
            });

            // ambil semua section
            var sections = rv.querySelectorAll('.cbi-section');

            // taruh setelah SYSTEM (biasanya index ke-1)
            if (sections.length > 1) {
                // ===== TARUH DI BAWAH SYSTEM (FIX DETEKSI) =====
var inserted = false;
...
sections.forEach(function(sec) {
    ...
    if (title && title.textContent.toLowerCase().includes('system') && !inserted) {
        sec.after(monitor);
        inserted = true;
    }
});
